//
//  NewRequireOrderViewController.swift
//  healthAndBeautyUser
//
...
//预约金优惠券缓存
var selectAppointmentDataSource = [NewSelectBookListModel]()
//尾款优惠券缓存
var selectFinalDataSource = [NewSelectBookListModel]()

//数据列表品
var RequireOrderDataSource = [NewStoreShopCarModel]()

class NewRequireOrderViewController: Wx_baseViewController {
    ...
    override func viewDidLoad() {
        ...
        createNaviController(title: "确认订单", leftBtn: buildLeftBtn(), rightBtn: nil)
    ...
    private func reBUildUI() {
        ...
        if isProject {
            ...
            for index in RequireOrderDataSource {
                ...
                tmpPrice1 += index.payPrice * Float(index.num)
                ...
                tmpPrice2 += index.retainage * Float(index.num)
        ...
        for num in 0..<RequireOrderDataSource.count {
            if isProject {
                let dic : NSDictionary = ["goodId":RequireOrderDataSource[num].id,
                                          "num":"\(RequireOrderDataSource[num].num)",
...
extension NewRequireOrderViewController: UITableViewDelegate {
...
extension NewRequireOrderViewController: UITableViewDataSource {
